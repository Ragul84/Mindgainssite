...
function renderPreviewQuestion(q, index) {
  return `<article class="question preview-question">
  ...
  for (const [examId, entries] of byExam) {
    ...
    for (const [subjectId, list] of subjectMap) {
      ...
      for (const t of list) {
        ...
        const preview = t.questions.slice(0, 3).map(renderPreviewQuestion).join('');
