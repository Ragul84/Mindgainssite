...
<header class="site-nav">
  ...
  <nav><a href="/blog/">Editorial</a><a href="/quiz/">Quiz Hub</a><a href="/#join">Waitlist</a></nav>
</header>
<main>${body}</main>
<script>
...
function articlePage(post, prev, next) {
  ...
  const breadcrumb = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 2, name: 'Editorial', item: SITE_URL + '/blog/' },
  ...
  const body = `<section class="blog-hero">
    <div class="breadcrumbs"><a href="/">Home</a><span>/</span><a href="/blog/">Editorial</a></div>
...
function indexPage(posts) {
  const body = `<section class="blog-hero"><p class="eyebrow">MindGains Editorial</p><h1>Ideas for India's daily learning habit.</h1><p class="dek">Research, product thinking and practical learning notes from the MindGains team.</p></section><section class="blog-list"><h2>Latest Articles</h2>${posts.map((post) => `<a class="blog-card" href="/blog/${post.slug}"><h2>${htmlEscape(post.title)}</h2><p>${htmlEscape(post.description)}</p><div class="meta-row"><span>${htmlEscape(post.author)}</span><span>Published ${htmlEscape(formatDate(post.publishedDate))}</span><span>${htmlEscape(post.readingTime)}</span></div></a>`).join('')}</section>`;
...
function updateHome() {
  ...
  html = html.replace(/<a href="\/blog\/">Blog<\/a>/g, '<a href="/blog/">Editorial</a>');
